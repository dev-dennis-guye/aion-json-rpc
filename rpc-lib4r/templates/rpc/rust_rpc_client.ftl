...
<#list methods as method>
    ...
    */
    public final ${macros.toRustType(method.returnType)} ${method.name}(<#list method.param.fields as parameter>${macros.toRustType(parameter.type)} ${parameter.fieldName}<#if parameter_has_next>,</#if></#list>){
        ${macros.toRustType(method.param)} params= new ${macros.toRustType(method.param)}(<#list method.param.fields as parameter>${parameter.fieldName}<#if parameter_has_next> ,</#if></#list>);
...
<#list methods as method>
    ...
    */
    public final <O> CompletableFuture<O> ${method.name}(<#list method.param.fields as parameter>${macros.toRustType(parameter.type)} ${parameter.fieldName},</#list> BiFunction<${macros.toRustType(method.returnType)}, RPCError, O> asyncTask){
        ${macros.toRustType(method.param)} params= new ${macros.toRustType(method.param)}(<#list method.param.fields as parameter>${parameter.fieldName}<#if parameter_has_next> ,</#if></#list>);
        Request request = new Request(generator.generateID(), "${method.name}", ${macros.toJavaConverter(method.param)}.encode(params), VersionType.Version2);
